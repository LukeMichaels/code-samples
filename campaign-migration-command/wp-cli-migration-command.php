...
class Campaign_Action_Migration_Command extends WP_CLI_Command {
   ...
   *
   * [--batch-size=<number>]
   * : Number of rows to process between cache flushes. Default 200.
   *
   * [--dry-run]
  ...
  public function import_actions( $args, $assoc_args ) {
    ...
    $total_rows = $this->count_data_rows( $file );

    WP_CLI::log( sprintf(
      'Importing %d row(s) from %s%s',
      $total_rows,
      $file,
    ...
    $progress = WP_CLI\Utils\make_progress_bar( 'Processing rows', $total_rows );
    ...
    while ( false !== ( $row = fgetcsv( $handle ) ) ) {
      ...
      if ( ! $dry_run && $since_flush >= $batch_size ) {
        wp_cache_flush();
        $since_flush = 0;
      }
    }
  ...
  private function count_data_rows( $file ) {
    $handle = fopen( $file, 'r' );
    $count  = -1; // The header row doesn't count as data.
